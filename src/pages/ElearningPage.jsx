...
export const ElearningPage = () => {
  ...
  const fetchSubscriptions = async () => {
    ...
    } catch (error) { console.error(error); } finally { setSubsLoading(false); }
  };
  useEffect(() => { fetchSubscriptions(); }, []);
  const handlePlanClick = (plan) => { setTargetPlan(plan); setIsDialogOpen(true); };
  const handleConfirmUpgrade = async (method) => {
    ...
    try {
      setIsUpgrading(true);
      await subscriptionService.upgradeSubscription(activeSub.id, { plan: targetPlan, payment_method: method });
      setIsDialogOpen(false); alert(`Berhasil upgrade ke ${targetPlan}!`); fetchSubscriptions();
    } catch (error) { alert(error.response?.data?.message || "Gagal upgrade."); } finally { setIsUpgrading(false); }
  };

  // --- QUERY DATA ---
  const { data: courses = [], isLoading: loadingCourses, isError } = useQuery({
    queryKey: ["courses"], queryFn: ElearningService.fetchCourses, staleTime: 1000 * 60 * 5,
  });
  const { data: recData, isLoading: loadingRecs } = useQuery({
    queryKey: ["recommendations", token], queryFn: ProfileService.getRecommendations, enabled: isLoggedIn, staleTime: 1000 * 60 * 5,
  });
  const { data: enrolledCourses = [] } = useQuery({
    queryKey: ["enrolled-courses", token], queryFn: async () => {
      if (!isLoggedIn) return [];
      try { const res = await ProfileService.getEnrolledCourses(); return Array.isArray(res) ? res : res.data || []; } catch { return []; }
    }, enabled: isLoggedIn, staleTime: 1000 * 60 * 2,
  });
  ...
  const currentEnrolledCourses = filteredEnrolled.slice((enrolledPage - 1) * ITEMS_PER_ROW, enrolledPage * ITEMS_PER_ROW);

  // 2. Discovery
  const rawDiscovery = isLoggedIn && Array.isArray(recData?.data) && recData.data.length > 0 ? recData.data : courses;
  const filteredDiscovery = filterData(rawDiscovery, searchDiscovery);
  const currentDiscoveryCourses = filteredDiscovery.slice((currentPage - 1) * ITEMS_PER_ROW, currentPage * ITEMS_PER_ROW);

  // 3. Popular
  const allRegular = courses.filter((c) => c.type === 'course');
  const filteredPopular = filterData(allRegular, searchPopular);
  const currentPopularCourses = filteredPopular.slice((popularPage - 1) * ITEMS_PER_ROW, popularPage * ITEMS_PER_ROW);

  // 4. Bootcamp
  const allBootcamp = courses.filter((c) => c.type === 'bootcamp');
  const filteredBootcamp = filterData(allBootcamp, searchBootcamp);
  const currentBootcampCourses = filteredBootcamp.slice((bootcampPage - 1) * ITEMS_PER_ROW, bootcampPage * ITEMS_PER_ROW);
  ...
  if (loadingCourses || (isLoggedIn && loadingRecs)) return <ElearningPageSkeleton />;
  ...
  const PaginationControl = ({ page, total, onPageChange }) => {
    ...
    return (
      <div className="container mx-auto px-6 py-4 flex justify-center items-center gap-4">
        <button onClick={() => onPageChange(page - 1)} disabled={page === 1} className="px-4 py-2 border rounded-md disabled:opacity-50 hover:bg-accent text-sm">Previous</button>
        <span className="text-sm font-medium">Page {page} of {total}</span>
        <button onClick={() => onPageChange(page + 1)} disabled={page === total} className="px-4 py-2 border rounded-md disabled:opacity-50 hover:bg-accent text-sm">Next</button>
      </div>
    );
  };
    ...
    <div className="min-h-screen flex flex-col">
      <main className="flex-1 pb-20">
            ...
            {enrolledCourses.length > 0 ? (
               <>
                 ...
                 {filteredEnrolled.length === 0 && searchEnrolled && (
                     <div className="text-center pb-10 text-gray-500 -mt-4">Tidak ditemukan kursus.</div>
                 )}
                 
                 <PaginationControl page={enrolledPage} total={Math.ceil(filteredEnrolled.length / ITEMS_PER_ROW)} onPageChange={setEnrolledPage} />
               </>
            ) : (
               ...
               <section className="px-6 py-12 container mx-auto">
                 ...
                 <p className="text-muted-foreground mb-6">Lanjutkan progres belajar Anda.</p>
                 <div className="w-full py-12 border-2 border-dashed border-gray-200 rounded-2xl flex flex-col items-center justify-center text-center gap-4 bg-gray-50/50">
                    <div className="bg-white p-4 rounded-full shadow-sm"><BookOpen className="h-8 w-8 text-gray-400" /></div>
                    <h3 className="text-lg font-semibold text-gray-900">Belum ada kursus yang diikuti</h3>
                    <p className="text-sm text-gray-500 max-w-sm mx-auto">Yuk, mulai perjalanan belajar Anda sekarang!</p>
                 </div>
               </section>
            )}
          </div>
        ...
        <div id="discovery-section" className="mt-8 scroll-mt-10">
          <ElearningList
            title={isLoggedIn ? "Rekomendasi Untuk Anda" : "Temukan Keahlian Baru"}
            subtitle={isLoggedIn ? "Kursus yang disesuaikan minat Anda." : "Perluas wawasan Anda."}
          ...
          {filteredDiscovery.length === 0 && searchDiscovery && (
              <div className="text-center pb-10 text-gray-500 -mt-4">Tidak ada kursus ditemukan.</div>
          )}
          <PaginationControl page={currentPage} total={Math.ceil(filteredDiscovery.length / ITEMS_PER_ROW)} onPageChange={setCurrentPage} />
        </div>

        <InfoSubscription subscriptions={subscriptions} loading={subsLoading} onSubscribe={handlePlanClick} />
        ...
        <div id="course" className="scroll-mt-10 mt-8">
          ...
          />
           {filteredPopular.length === 0 && searchPopular && (
              <div className="text-center pb-10 text-gray-500 -mt-4">Tidak ada kursus populer ditemukan.</div>
          )}
          <PaginationControl page={popularPage} total={Math.ceil(filteredPopular.length / ITEMS_PER_ROW)} onPageChange={setPopularPage} />
        </div>
        ...
        <div id="bootcamp" className="scroll-mt-10 mt-8">
          ...
          />
           {filteredBootcamp.length === 0 && searchBootcamp && (
              <div className="text-center pb-10 text-gray-500 -mt-4">Tidak ada bootcamp ditemukan.</div>
          )}
          <PaginationControl page={bootcampPage} total={Math.ceil(filteredBootcamp.length / ITEMS_PER_ROW)} onPageChange={setBootcampPage} />
        </div>
