...
import { 
  ...
  Building2, Globe, Mail, Phone, BookOpen, Star, CheckCircle2, XCircle, Clock
} from "lucide-react";
...
export const ViewScholarshipDialog = ({ open, onOpenChange, scholarshipId }) => {
  ...
  // Ambil data dari wrapper .data sesuai JSON kamu
  ...
  const { data: rawApplicants, isLoading: loadingApplicants } = useQuery({
    ...
    queryFn: () => ScholarshipService.getScholarshipApplicants(scholarshipId),
    enabled: !!scholarshipId && open,
    staleTime: 0,
  });
  ...
  const applicants = Array.isArray(rawApplicants) 
    ? rawApplicants 
    : (Array.isArray(rawApplicants?.data) ? rawApplicants.data : []);

  // 3. MUTATION STATUS
  const statusMutation = useMutation({
    mutationFn: ({ applicationId, status }) => 
        ScholarshipService.updateApplicantStatus(applicationId, status),
    onSuccess: () => {
        ...
        {loadingDetail ? (
            ...
            <Tabs defaultValue="detail" className="flex-1 flex flex-col overflow-hidden">
                ...
                <TabsContent value="detail" className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/30 m-0">
                    ...
                    {/* 2. Informasi Organisasi (Provider) */}
                            ...
                            <div className="flex flex-col md:flex-row gap-5 items-start">
                                ...
                                <div className="w-20 h-20 shrink-0 border rounded-lg p-2 flex items-center justify-center bg-white shadow-sm">
                                    <img 
                                        ...
                                        onError={(e) => {e.target.src = "https://via.placeholder.com/100?text=ORG"}} // Fallback image
                                    />
                                </div>
                                ...
                                <div className="flex-1 space-y-2">
                                    ...
                                    <div className="flex flex-wrap gap-4 pt-2">
                                        ...
                                        {scholarship.organization.phone && (
                                            <div className="flex items-center gap-1.5 text-xs font-medium text-neutral-600 bg-neutral-100 px-2 py-1 rounded-md">
                                                <Phone className="w-3.5 h-3.5" /> {scholarship.organization.phone}
                                            </div>
                                        )}
                                    </div>
                                </div>
                                    ...
                                    <div key={app.id} className="flex flex-col md:flex-row md:items-center justify-between p-4 border rounded-xl bg-white shadow-sm hover:shadow-md transition-all gap-4">
                                        <div className="flex gap-4 overflow-hidden">
                                            <Avatar className="h-12 w-12 border-2 border-white shadow-sm">
                                                <AvatarImage src={app.student?.avatar} />
                                                <AvatarFallback className="bg-primary text-white font-bold">
                                                    {app.student?.name?.substring(0,2).toUpperCase()}
                                                </AvatarFallback>
                                            </Avatar>
                                            <div className="min-w-0 flex flex-col justify-center">
                                                <h4 className="font-bold text-sm text-neutral-800 truncate">{app.student?.name}</h4>
                                                <p className="text-xs text-neutral-500 truncate">{app.student?.email}</p>
                                                <div className="flex items-center gap-2 mt-1.5">
                                                    <span className="text-[10px] font-medium text-neutral-500 bg-neutral-100 px-2 py-0.5 rounded-full">
                                                        Applied: {formatDate(app.created_at)}
                                                    </span>
                                                    {app.cv_path && (
                                                        <a href={app.cv_path} target="_blank" rel="noreferrer" className="text-[10px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full hover:bg-blue-100 font-medium flex items-center gap-1 transition-colors">
                                        ...
                                        <div className="flex flex-row md:flex-col items-center md:items-end gap-3 md:gap-2 shrink-0 ml-auto">
                                            ...
                                            {app.status === 'pending' && (
                                                <div className="flex gap-1">
                                                    <Button size="sm" variant="outline" className="h-7 w-7 p-0 text-green-600 border-green-200 hover:bg-green-50" onClick={() => handleStatusChange(app.id, 'accepted')}><Check className="h-3.5 w-3.5" /></Button>
                                                    <Button size="sm" variant="outline" className="h-7 w-7 p-0 text-red-600 border-red-200 hover:bg-red-50" onClick={() => handleStatusChange(app.id, 'rejected')}><X className="h-3.5 w-3.5" /></Button>
                                                </div>
                                            )}
                                        </div>
                                    </div>
