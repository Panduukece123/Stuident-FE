...
// Import komponen tab & dialog yang baru dibuat
...
export const SessionDetail = ({ session, onBack }) => {
  const [showFeedback, setShowFeedback] = useState(false);

  if (!session) return null;

  // Formatting helpers (sama seperti sebelumnya)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="w-full flex flex-col md:flex-row md:items-center justify-between bg-transparent border-b-2 border-b-primary py-2 gap-4">
        ...
        >
          ...
          <h1 className="text-xl text-neutral-800">
            Detail Sesi {session.session_id}
          </h1>
        </div>
        ...
        <div className="flex items-center gap-3">
            ...
            {/* Tombol Feedback (Muncul jika selesai) */}
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        {/* TAB 1: OVERVIEW (Info Profil & Jadwal yang lama) */}
        <TabsContent value="overview" className="space-y-6">
           <Card>
            ...
            <CardContent className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
               {/* ... (Isi card jadwal sama persis seperti kode sebelumnya) ... */}
          ...
          <div className="grid gap-6 md:grid-cols-2">
             {/* ... (Isi profil Mentor & Student sama persis seperti kode sebelumnya) ... */}
              ...
              <div className="space-y-3">
                <h3 className="flex items-center gap-2 text-sm font-semibold text-neutral-900"><GraduationCap className="h-4 w-4" /> Student Profile</h3>
                 <Card className="bg-neutral-50/50"><CardContent className="p-6 space-y-4">
                    <div className="flex items-center gap-4">
        ...
        <TabsContent value="assessment">
           <NeedAssessmentTab sessionId={session.id} />
        </TabsContent>

        {/* TAB 3: FILES */}
        <TabsContent value="files">
           <CoachingFilesTab sessionId={session.id} />
        </TabsContent>
      </Tabs>
