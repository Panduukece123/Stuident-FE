...
const MentoringService = {
  ...
  getMentorSchedule: async (mentorId, params = {}) => {
    ...
    const response = await api.get(`/mentors/${mentorId}/schedule`, {
      params: {
  ...
  submitFeedback: async (id, payload) => {
    ...
    const response = await api.post(
      `/mentoring-sessions/${id}/feedback`,
      payload,
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  getNeedAssessment: async (id) => {
    ...
    const response = await api.get(
      `/mentoring-sessions/${id}/need-assessments`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
  ...
  updateStatus: async (sessionId, status) => {
    const token = localStorage.getItem("token");
    const response = await api.put(
      `/mentoring-sessions/${sessionId}/status`,
      {
        status: status,
      },
      {
  ...
  markAssessmentCompleted: async (sessionId) => {
    const token = localStorage.getItem("token");
    const response = await api.put(
      `/mentoring-sessions/${sessionId}/need-assessments/mark-completed`,
      {},
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  uploadCoachingFile: async (sessionId, file) => {
    ...
    const response = await api.post(
      `/mentoring-sessions/${sessionId}/coaching-files`,
      formData,
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  deleteCoachingFile: async (sessionId, fileId) => {
    const token = localStorage.getItem("token");
    const response = await api.delete(
      `/mentoring-sessions/${sessionId}/coaching-files/${fileId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
